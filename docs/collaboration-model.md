...
- When a doc and a source file disagree: Committed sim runs and source win over docs; docs/current-state.md wins over the journal. When a doc and a sim result disagree, re-run the sim at HEAD (reproducibility is the contract) and fix the doc in the same session.
- Staleness review cadence: Every wake lands on origin/main HEAD before reading anything. The status heartbeat is the only readable record of a wake (owner-verified: completed routine runs are not inspectable) — a heartbeat older than two wake cycles (>4h) is stale and gets flagged in the next overwrite. sims/REFERENCE.md is re-checked whenever the harness tag bumps.
