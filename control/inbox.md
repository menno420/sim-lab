...
## ORDER 006 · 2026-07-14T04:10:25Z · status: new
priority: P2
do: (1) roll over control/outbox.md (~875KB, measured 1,016,968 B at sim-lab HEAD 995cf51) per fleet-manager docs/conventions/outbox-rollover.md: 200KB threshold · terminal-blocks-only into a dated archive file · mandatory pointer stub before the roll · content-stable numbering — this is the convention answer ORDER 005 item 5 was parked awaiting; (2) cross-link the OA-002 split verdict — enabled=resolved / quota=open (INC-04) — into the lane's ledger (docs/current-state.md OA-002 entries currently carry only "usage-capped / quota still open", no split disposition).
why: the outbox is ~4× the fleet rollover threshold and the OA-002 disposition lives only fm-side; both accepted at the fm 2026-07-14 overnight wake (fm main @ 780c81b). Provenance: relayed by the Fleet Manager seat, coordinator dispatch 2026-07-14, fm docs/dispatch-log.md @ 780c81b.
done-when: control/outbox.md under 200KB with archive + pointer stub; the ledger entry cites OA-002 with the split verdict.

## ORDER 007 · 2026-07-14T05:40:16Z · status: new
priority: routine
do: fold this repo's real walls into the `substrate-kit:capability-seed` fenced walls section of docs/CAPABILITIES.md so the seat-digest walls block stops rendering empty. Two sources to fold: (1) the un-tagged "## Walls — verified blocked" section already in docs/CAPABILITIES.md (line 47 at HEAD 09782df) — move/venue-tag its entries into the fenced section in the kit's extractable form; (2) root-level PLATFORM-LIMITS.md — fold its verified walls in too (PLATFORM-LIMITS.md may stay as a pointer stub afterward — lane's call). The capability-seed fence markers are ABSENT from docs/CAPABILITIES.md (0 grep hits at 09782df) — restore/add them (reference shape: superbot-next docs/CAPABILITIES.md:19/:101 carries the fence). Regenerate the digest (`python3 bootstrap.py seat-digest`) after the fold.
why: fm fence-exposure index (fm docs/fence-index.md@3b335a86f49f3a35d40ba12ee06bdba3bdfd87bc, Phase 1 B4) found sim-lab@09782dfdf55202986e048e11226746bcb8c2b66d with an EMPTY walls digest (docs/seat-digest.md:38 "(no walls recorded for these venues in `docs/CAPABILITIES.md`)") while real walls sit outside the fence — fleet tooling reports the seat wall-free when it isn't.
done-when: fenced walls section present and non-empty at main HEAD, reflecting both sources (un-tagged CAPABILITIES walls + PLATFORM-LIMITS.md); walls digest renders non-empty; `python3 bootstrap.py check --strict` green.
Provenance: relayed by the Fleet Manager seat, coordinator dispatch 2026-07-14, fm docs/dispatch-log.md @ 3b335a8.

## ORDER 008 · 2026-07-14T09:34:40Z · status: new
...
do:
  (a) FINISH — today (2026-07-14) is the EAP final day. Complete what is completable today from this cited list; anything that can't finish gets parked HONESTLY with a one-line citation of why: (1) one heartbeat PR closing the drift — done= flips for ORDERs 005/006/007 (all executed: #111-era items + kit v1.15.0; rollover #134 `5356b2a`; capability-fence + digest #136 `b10ffce`) + ledger refresh through V073 (V069–V073 merged via #128/#131/#132/#133/#135 while the ledger stops at V068+V069-in-flight); (2) NEW ROUTING (per Q-0264, via this ORDER): superbot-games filed two SIM-REQUESTs (fishing-full-roster + cook-leg, games PR #92 commit `21937f3`) that never reached this inbox — verdict them today if feasible, else park cited; (3) VERDICT 074 when idea-engine P063 arrives (its ORDER 012 directs drafting it today; pipeline dry at HEAD, P062→V073 done at the constant +11 offset). Parked (cite these): OA-002 Codex usage cap (owner click, INC-04, split verdict enabled=resolved/quota=open); OA-003 review-site deploy (owner/websites-lane click, VERDICT 011); OA-004 harness tag-push 403 (owner grants refs/tags/* or pushes harness-v0.1.0; non-blocking). Premises are from fm recon at `2ceeb59628d2897964ddee3d2fd58cbaa41cfce0` — re-verify each live before acting (Q-0120).
