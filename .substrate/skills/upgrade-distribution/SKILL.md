...
Roll a substrate-kit release out to sim-lab — one target repo of the
...
7. Verify + flip — `python3 bootstrap.py check --strict (the substrate gate); plus each touched sim's own documented run command reproducing its committed result` and
