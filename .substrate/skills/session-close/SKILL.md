...
Land sim-lab's session correctly — the full landing path, claim to
...
6. Verify — `python3 bootstrap.py check --strict (the substrate gate); plus each touched sim's own documented run command reproducing its committed result` and `python3 bootstrap.py check --strict`.
