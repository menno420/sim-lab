# substrate-kit upgrade report — v1.7.0 → v1.15.0

> Generated 2026-07-13 by `bootstrap.py upgrade`. Rollback: `python3 bootstrap.py upgrade --rollback`.

**Docs:** consumer-edited: 2 · diverged: 2 · missing: 4 · template-improved: 5 · unchanged: 10
...
- carve-out: full pre-regen enabler banked at .substrate/backup/auto-merge-enabler.pre-regen-189497a9.yml — host additions were NOT carried into the regenerated kit-owned enabler; move them into a separate workflow file (e.g. .github/workflows/host-ci.yml) and commit that before shipping this upgrade/adopt PR.
...
- carve-out scan: .github/workflows/auto-merge-enabler.yml — previous kit template unavailable (no staged copy to recover it from); two-way compare vs the new template may report kit-side template changes as host additions
